...
/**
 * 2021-10-26
 *  pagination을 적용하기위하여 controller method를 변경한다
 *   전체리스트를 보여주는 list() 함수를 변경한다
 *   가. list() 함수에 pagination을 적용하기 위한 매개변수를 설정한다
 */
@Controller
...
//  private val logger = LoggerFactory.getLogger(BuyerController::class.java)
     ...
     *  Pageable pageable 
     *   JPA의 기능을 사용하여 paginaation을 구현하기 위한 하나의 도구
     ...
     *      &sort=name,ase
     *
     *  만약 주문정보를 조회한다면 다음처럼 최근 입력한 자료가 먼저 나타나도록 할때
     *    http://localhost:8080/erp/buyer/list
     *      ?page=10
     *      &size=10
     *      &sort=date,desc
    ...
    fun list(model: Model, @PathVariable("page")page:String="0"): String {
        ...
        logger().debug("여기는 list 함수")
        val buyerList = bService.selectWithPageble(intPage)
