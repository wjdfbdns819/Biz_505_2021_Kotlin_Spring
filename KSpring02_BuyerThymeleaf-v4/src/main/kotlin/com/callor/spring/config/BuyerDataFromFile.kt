...
class BuyerDataFromFile(val buyerDao : BuyerRepository) {
    ...
    /**
     * @Value()
     *  *.properties 또는 *.yml 파일에 설정된 속성(com.을 참조하여
     *   속성에 설정된 값(주로 문자열)을 가져와서 dataFile 변수에 담아라
     */
    ...
    fun dataFromFile():CommandLineRunner {
        ...
        val managerList =  fileList.map {
            ...
            // 따로 변수를 선언하지 않으면 it에 담김
            val lines = it.split(":")
            
            // {이름 : "홍길동", 전화번호: "010-111-1111"} 형식의 데이터를 생성
